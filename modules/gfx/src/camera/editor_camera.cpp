...
EditorCamera::Builder& EditorCamera::Builder::SetEventBus(const ref<events::EventBus>& bus) noexcept {
    desc_.eventBus = bus;
    return *this;
}
...
void EditorCamera::SubscribeEvents() {
    ...
    connMouseMove_ = eventBus_->On<events::MouseMovedEvent>(
        [this](const events::MouseMovedEvent& e) {
            OnMouseMoved(e);
            return false;
        }
    );
    connMouseScroll_ = eventBus_->On<events::MouseScrolledEvent>(
        [this](const events::MouseScrolledEvent& e) {
            OnMouseScrolled(e);
            return false;
        }
    );
    connMouseButtonPress_ = eventBus_->On<events::MouseButtonPressedEvent>(
        [this](const events::MouseButtonPressedEvent& e) {
            OnMouseButtonPressed(e);
            return false;
        }
    );
    connMouseButtonRelease_ = eventBus_->On<events::MouseButtonReleasedEvent>(
        [this](const events::MouseButtonReleasedEvent& e) {
            OnMouseButtonReleased(e);
            return false;
        }
    );
    connKeyPress_ = eventBus_->On<events::KeyPressedEvent>(
        [this](const events::KeyPressedEvent& e) {
            OnKeyPressed(e);
            return false;
        }
    );
    connKeyRelease_ = eventBus_->On<events::KeyReleasedEvent>(
        [this](const events::KeyReleasedEvent& e) {
            OnKeyReleased(e);
            return false;
        }
    );
    connViewportResize_ = eventBus_->On<events::ViewportResizeEvent>(
        [this](const events::ViewportResizeEvent& e) {
            OnViewportResize(e);
            return false;
        }
    );
...
void EditorCamera::OnMouseMoved(const events::MouseMovedEvent& e) {
    ...
    if (orbiting_) {
        ...
        cc::vec3f forward = (camera_.GetTarget() - camera_.GetPosition()).norm();
...
void EditorCamera::OnMouseScrolled(const events::MouseScrolledEvent& e) {
    f32 dir = -e.offsetY;
...
void EditorCamera::OnMouseButtonPressed(const events::MouseButtonPressedEvent& e) {
...
void EditorCamera::OnMouseButtonReleased(const events::MouseButtonReleasedEvent& e) {
    if (e.button == config_.orbitButton) {
        orbiting_ = false;
    } else if (e.button == config_.panButton) {
        panning_ = false;
...
void EditorCamera::OnKeyPressed(const events::KeyPressedEvent& e) {
    if (e.key == config_.fastModifier) {
        fastMode_ = true;
    }
}

void EditorCamera::OnKeyReleased(const events::KeyReleasedEvent& e) {
    if (e.key == config_.fastModifier) {
        fastMode_ = false;
    }
}

void EditorCamera::OnViewportResize(const events::ViewportResizeEvent& e) {
